...
<div class="row">
    <div class="col-sm-12">
        <a class="btn btn-primary" href="<?=\yii\helpers\Url::to(['user/orders'])?>"><?= Yii::t('app','Back to orders')?></a>
        <?php if ($model->status=Order::STATUS_APPROVED) { ?>
            &nbsp;
            <a target="_blank" class="btn btn-primary" href="<?=\yii\helpers\Url::to(['user/order','id'=>$model->id, 'output'=>'print'])?>" target="_blank"><span class="glyphicon glyphicon-print"></span> <?= Yii::t('app','Print Invoice')?></a>
        <?php } ?>
    </div>
</div>